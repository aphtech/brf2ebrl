...
def create_centered_detector(cells_per_line: int, min_indent: int, tag_name: str) -> Detector:
    """Creates a detector for detecting centered text."""
    def detect_centered(text: str, cursor: int, state: DetectionState, output_text: str) -> DetectionResult:
        lines = []
        new_cursor = cursor
        while line := re.search(f"^(\u2800{{{min_indent},}})([\u2801-\u28ff][\u2800-\u28ff]*)[\n\f]+", text[new_cursor:]):
            ...
            if len(line.group(1)) in indents:
                ...
                break
        brl = "\u2800".join(lines)
        return DetectionResult(new_cursor, state, 0.9, f"{output_text}<{tag_name}>{brl}</{tag_name}>\n") if brl else DetectionResult(cursor + 1, state, 0.0, output_text + text[cursor])
